...
import { useCreateEnclaveClassTemplate } from '../api/createEnclaveClassTemplate';
...
export const CreateEnclaveClassTemplate = () => {
  const navigate = useNavigate();
  const { data: nodesData } = useNodeClassTemplates();
  const { data: enclavesData } = useEnclaveClassTemplates();
  const { mutate } = useCreateEnclaveClassTemplate();

  const handleSubmit = (data: EnclaveClassTemplate) =>
    mutate(
      { data },
      {
        onSuccess: () => navigate('..'),
