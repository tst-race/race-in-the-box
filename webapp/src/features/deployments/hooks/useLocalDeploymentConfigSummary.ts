...
export type UseLocalDeploymentConfigSummaryOptions = {
  name: string;
};

export type UseLocalDeploymentConfigSummaryResult = {
  info: LocalDeploymentInfo | null;
  isLoading: boolean;
  nodeCounts: NodeCounts;
};

export const useLocalDeploymentConfigSummary = ({
  name,
}: UseLocalDeploymentConfigSummaryOptions) => {
  const { data, isLoading } = useLocalDeploymentInfo({ name });
