...
export type LocalDeploymentConfigFactsProps = {
  name: string;
};

export const LocalDeploymentConfigFacts = ({ name }: LocalDeploymentConfigFactsProps) => {
  const { info, isLoading, nodeCounts } = useLocalDeploymentConfigSummary({ name });
  ...
  return (
    <>
      ...
      <H4>Images</H4>
      <DescriptionList margin pending={isLoading} striped weight={4}>
        {info?.config.images.map((config) => (
          <Fragment key={`${config.platform}-${config.architecture}-${config.node_type}}`}>
            <dt>{`${config.platform} ${config.architecture} ${config.node_type}`}</dt>
            <dd>
              <CopyableText code text={config.tag} />
            </dd>
          </Fragment>
        ))}
      </DescriptionList>
      ...
      <DescriptionList margin pending={isLoading} striped weight={4}>
        ...
        <dt>Android Container Acceleration</dt>
        <dd>{info?.config.android_container_acceleration ? 'yes' : 'no'}</dd>
        <dt>Size of tmpfs</dt>
        <dd>{info?.config.tmpfs_size}</dd>
      </DescriptionList>
    </>
  );
};
