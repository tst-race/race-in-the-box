...
formatlog "INFO" "Building Graph Renderer Image: ghcr.io/tst-race/race-in-the-box/graph_renderer:main"
DOCKER_BUILDKIT=1 docker build \
    ${CACHE_ARG:+'--no-cache'} \
    --build-arg RIB_VERSION="${RIB_VERSION}" \
    -f "${FILE_DIR}/Dockerfile" \
    -t ghcr.io/tst-race/race-in-the-box/graph_renderer:main \
    --cache-from ghcr.io/tst-race/race-in-the-box/graph_renderer:main \
    --build-arg BUILDKIT_INLINE_CACHE=1 \
    "${DOCKER_LABEL_ARGS[@]}" \
    "${FILE_DIR}"
formatlog "INFO" "Built image: ghcr.io/tst-race/race-in-the-box/graph_renderer:main"
