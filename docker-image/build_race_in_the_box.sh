...
#
# Example Call:
#    bash build_race_in_the_box.sh {--version=1.0.0} {--label=testing} {--no-cache}
# -----------------------------------------------------------------------------
...
formatlog "INFO" "Building RACE in the Box Image: race-in-the-box:${RIB_VERSION}"
DOCKER_BUILDKIT=1 docker build \
    --progress=plain \
    ${CACHE_ARG:+'--no-cache'} \
    --build-arg RIB_VERSION="${RIB_VERSION}" \
    -f "${FILE_DIR}/Dockerfile" \
    -t race-in-the-box:"${RIB_VERSION}" \
    --cache-from race-in-the-box:"${RIB_VERSION}" \
    --build-arg BUILDKIT_INLINE_CACHE=1 \
    "${DOCKER_LABEL_ARGS[@]}" \
    "${FILE_DIR}/../"
formatlog "INFO" "Built image: race-in-the-box:${RIB_VERSION}"
